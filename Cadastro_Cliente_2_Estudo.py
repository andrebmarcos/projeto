...
class Funcs():
    ...
    def desconectar_db(self):
        self.conn.close()
        print("Desonectando do banco de dados")
    ...
    def select_lista(self):
        ...
        lista = self.cursor.execute(
            """SELECT cod, nome_cliente, telefone, cidade FROM clientes2 ORDER BY nome_cliente ASC; """)
        for i in lista:
    ...
    def deleta_cliente(self):
        ...
        self.conecta_db()
        self.cursor.execute(
            """DELETE FROM clientes2 WHERE cod = ? """, (self.codigo))
        self.conn.commit()
        self.desconectar_db()
        self.limpa_tela()
        self.select_lista()
    ...
    def alterar_cliente(self):
        self.variaveis()
        self.conecta_db()
        self.cursor.execute(""" UPDATE clientes2 SET nome_cliente = ?, telefone = ?, cidade = ? WHERE cod = ? """, (self.nome,self.telefone, self.cidade, self.codigo ))
        self.conn.commit()
        self.desconectar_db()
        self.select_lista()
        self.limpa_tela()
...
# Criando Botoes
    def widgets_frame1(self):
        ...
        self.bt_limpar = Button(self.frame1, text='Limpar', bd=2, bg='#107db2', fg='white', font=(
    ...
    # criando botãi buscar
        self.bt_buscar = Button(self.frame1, text='Buscar', bd=2,
                                bg='#107db2', fg='white', font=('verdana', 8, 'bold'))
        self.bt_buscar.place(relx=0.3, rely=0.13, relwidth=0.1, relheight=0.12)
    # criando botão novo
        self.bt_novo = Button(self.frame1, text='Novo', bd=2, bg='#107db2', fg='white', font=(
            'verdana', 8, 'bold'), command=self.add_cliente)
        self.bt_novo.place(relx=0.6, rely=0.13, relwidth=0.1, relheight=0.12)
    # criando botãi alterar
        self.bt_alterar = Button(self.frame1, text='Alterar', bd=2,
                                 bg='#107db2', fg='white', font=('verdana', 8, 'bold'), command = self.alterar_cliente)
        self.bt_alterar.place(relx=0.7, rely=0.13,
                              relwidth=0.1, relheight=0.12)
    # criando botãi apagar
        self.bt_apagar = Button(self.frame1, text='Apagar', bd=2,
    ...
    # Criação da label e entrada do codigo
        ...
        self.lb_telefone = Label(
            self.frame1, text='Telefone', bd=4, bg='#dfe3ee', fg='#107db2')
